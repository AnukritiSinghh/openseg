...
class ClipAdapter(nn.Module):
    ...
    def forward(self, image: torch.Tensor, text: List[str], **kwargs):
        ...
        features_unmasked = features_unmasked.unsqueeze(2)
        ...
        #exit()
        #features_masked = self.get_image_features(image2)
    ...
    def _get_text_features(self, noun_list: List[str], features_unmasked=None):
        if self.prompt_learner.with_conditional_trainable_params:
            text_features = self.prompt_learner(noun_list, self.clip_model, features_unmasked)
    ...
    def get_text_features(self, noun_list: List[str], features_unmasked):
        return self._get_text_features(noun_list, features_unmasked)
