...
class ConditionalLearnablePromptExtractor(PromptExtractor):
    def __init__(self, prompt_dim: int, prompt_shape: Tuple[int, int], clip_model):
        super().__init__()
        ...
        if cfg.TRAINER.COCOOP.PREC == "fp16":    # change the cgf file location  # CLIP's default precision is fp16
            self.meta_net.half()
    ...
    def forward(self, noun_list: List[str], clip_model: nn.Module):
        ...
        features = self.backbone(images.tensor)    # these are the image features which will be used be in met_net
        ...
        bias = self.meta_net(features)  # (batch, ctx_dim)   #features are coming from line 112
